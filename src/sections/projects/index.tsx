...
export default function Projects() {
  ...
  const [projectsTabOpened, setProjectsTabOpened] = React.useState<boolean>(
    window.innerWidth < 1420 ? false : true,
  );
    ...
    >
      ...
      <Presentation>
        {projects[currentProj] ? (
          <ProjDisplay project={projects[currentProj]}></ProjDisplay>
        ) : (
          ""
        )}
      </Presentation>
      ...
      <Decoration
        as={motion.div}
        animate={{ transform: "rotate(-10deg) translateX(100px)" }}
      ...
      {window.innerWidth < 1420 && (
        <OpenButton
          $isOpened={projectsTabOpened}
          onClick={() => setProjectsTabOpened(!projectsTabOpened)}
        ></OpenButton>
      )}
    </Main>
  );
